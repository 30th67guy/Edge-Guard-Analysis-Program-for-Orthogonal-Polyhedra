...
int main() {   
    // Settings
    const int NUM_SAMPLES = 1; // 試行回数 (データを取るために多めに設定)
    settings config;
    // -----------------config-----------------
    // General (3-Reflex):  shape = 0, mode = 1
    // 1-Reflex (Prism):    shape = 1, mode = 1
    // 2-Reflex (Terrain):  shape = 1, mode = 0
    // ----------------------------------------
    config.shape = 1; 
    config.mode = 1;  
    config.amount = 200;
    ...
    // For saving statistic data
    std::vector<double> efficiency_list; // 各回の効率 (G / (m/6)) を保存

    // For recording to a file
    std::ofstream csvFile("experiment_results.csv");

    csvFile << "Sample_ID,Real_Edges_m,Reflex_Edges_r,Convex_Edges_c,Guards_G,"
            << "m/6,Eff_m/6,"              // (1) m/6
            << "(11/72)m,Eff_(11/72)m,"    // (2) (11/72)m
            << "(7/12)r,Eff_(7/12)r,"      // (3) (7/12)r
            << "(m+r)/12,Eff_(m+r)/12,"  // (4) (m+r)/12  <- (e+r)/12
            << "m/12_Prism,Eff_m/12_Prism\n";      // (5) m/12 (参考: 1-reflex用)
    
    std::cout << "Starting data collection (" << NUM_SAMPLES << " samples)..." << std::endl;
    if(config.shape == 0 && config.mode == 1) std::cout << "Type: General (3-Reflex)" << std::endl;
    else if(config.shape == 1 && config.mode == 1) std::cout << "Type: 1-Reflex (Prism)" << std::endl;
    else if(config.shape == 1 && config.mode == 0) std::cout << "Type: 2-Reflex (Terrain)" << std::endl;
    auto total_start = std::chrono::system_clock::now();

    // データ収集loop
    for (int sample_idx = 0; sample_idx < NUM_SAMPLES; ++sample_idx){
        
        // 1. Print progress (every 100 times)
        if ((sample_idx + 1) % 10 == 0) {
            std::cout << "Processing sample " << (sample_idx + 1) << " / " << NUM_SAMPLES << "..." << std::endl;
        }

        // 2. Creation of cuboids
        Nef_polyhedron cubes;
        
        // For 2-reflex, the range of creation of cuboids
        double min_x = 10000, max_x = -10000;
        double min_z = 10000, max_z = -10000;
        
        // For 1,2-Reflex
        double fixed_height = get_rand(100, 500, 800, mt_engine);

        // Create a first cuboid
        Nef_polyhedron init_cube;
        if(config.shape == 0) {
            // General: random
            init_cube = make_cube(Point(0, 0, 0), get_rand(100, 500, 800, mt_engine), get_rand(100, 500, 800, mt_engine), get_rand(100, 500, 800, mt_engine));
        } else {
            // 1,2-Reflex: fix their height
            double start_y = (config.mode == 0) ? fixed_height : 0.0;
            double len = get_rand(100, 500, 800, mt_engine);
            double dep = get_rand(100, 500, 800, mt_engine);
            init_cube = make_cube(Point(0, start_y, 0), len, fixed_height, dep);
        ...
        for (int i = 0; i < config.amount; i++) {
            // common part for each vertex
            double x = get_rand(100, 500, 1000, mt_engine) * xyz(mt_engine);
            double z = get_rand(100, 500, 1000, mt_engine) * xyz(mt_engine);
            
            // update the range
            double len = get_rand(100, 300, 800, mt_engine);
            double dep = get_rand(100, 300, 800, mt_engine);
            ...
            Nef_polyhedron new_shape;

            if (config.shape == 0) {
                // General Orthogonal Polyhedra
                double y = get_rand(100, 500, 1000, mt_engine) * xyz(mt_engine);
                double wid = get_rand(100, 300, 800, mt_engine);
                new_shape = make_cube(Point(x, y, z), len, wid, dep);
                ...
                // 1,2-Reflex (Height Fixed)
                double start_y = (config.mode == 0) ? fixed_height : 0.0;
                new_shape = make_cube(Point(x, start_y, z), len, fixed_height, dep);

                if (config.mode == 1) { 
                    // 1-Reflex: Enable Intersection Check (if any intersection, then connect them)
                    if (!cubes.intersection(new_shape).is_empty()) {
                        cubes += new_shape;
                        rate += 50;
                    } 
                    ...
                    // 2-Reflex: Enable Intersection Check (if no intersection, then connect them)
                    if (cubes.intersection(new_shape).is_empty()) {
                        cubes += new_shape;
                        rate += 50;
                    }
        ...
        // For 2-Reflex, add a bottom cube
        if (config.shape == 1 && config.mode == 0) {
            ...
            Nef_polyhedron bottom_cube = make_cube(Point(center_x, 0, center_z), base_width, fixed_height, base_depth);
            cubes += bottom_cube;
        }
        ...
        // 5. ガード数 G の算出 (12セットの中で最小のものを採用)
        const auto target_sets = define_target_pattern_sets();
        int min_guards_G = -1;

        for (const auto& current_set : target_sets) {
            if (current_set.size() != 3) continue;
            auto it = current_set.begin();
            CornerPattern p1 = *it; ++it; CornerPattern p2 = *it; ++it; CornerPattern p3 = *it;
            int g = (pattern_counts.count(p1) ? pattern_counts.at(p1) : 0) + 
                    (pattern_counts.count(p2) ? pattern_counts.at(p2) : 0) + 
                    (pattern_counts.count(p3) ? pattern_counts.at(p3) : 0);
            if (min_guards_G == -1 || g < min_guards_G) min_guards_G = g;
        }

        // --- 複数の上界と効率の計算 ---
        double m = static_cast<double>(real_edge_count);
        double m_r = static_cast<double>(reflex_edge_count);
        double m_c = static_cast<double>(convex_edge_count);
        
        if (m > 0) {
            // 1. m/6
            double ub_1 = m / 6.0;
            double eff_1 = min_guards_G / ub_1;

            // 2. (11/72)m
            double ub_2 = (11.0 / 72.0) * m;
            double eff_2 = min_guards_G / ub_2;

            // 3. (7/12)r
            double ub_3 = (7.0 / 12.0) * m_r;
            double eff_3 = (m_r > 0) ? (min_guards_G / ub_3) : 0.0;

            // 4. (m+r)/12 (Viglietta Sensei mentioned)
            double ub_4 = (m + m_r) / 12.0;
            double eff_4 = min_guards_G / ub_4;

            // 5. m/12 (For 1-reflex)
            double ub_5 = m / 12.0;
            double eff_5 = min_guards_G / ub_5;

            efficiency_list.push_back(eff_4);
    ...
    csvFile.close();
    std::cout << "Data saved to experiment_results.csv" << std::endl;

    // 統計的な計算の部分
    double sum = std::accumulate(efficiency_list.begin(), efficiency_list.end(), 0.0);
    double mean = sum / efficiency_list.size();

    double sq_sum = 0.0;
    for (double val : efficiency_list) {
        sq_sum += (val - mean) * (val - mean);
    }
    double variance = sq_sum / efficiency_list.size();
    ...
    // 結果表示
    std::cout << "\n==========================================" << std::endl;
    std::cout << "DATA COLLECTION RESULTS" << std::endl;
    std::cout << "==========================================" << std::endl;
    std::cout << "Samples processed: " << efficiency_list.size() << std::endl;
    std::cout << "Total time:        " << total_elapsed.count() << " sec" << std::endl;
    std::cout << "------------------------------------------" << std::endl;
    std::cout << "Efficiency (Measured G / Theoretical Upper Bound)" << std::endl;
    std::cout << "  Average (Mean):  " << mean << std::endl;
    std::cout << "  Variance:        " << variance << std::endl;
    std::cout << "  Std Deviation:   " << std_dev << std::endl;
    std::cout << "  Used efficiency:   " << "(m+r)/12" << std::endl;
    std::cout << "==========================================" << std::endl;
